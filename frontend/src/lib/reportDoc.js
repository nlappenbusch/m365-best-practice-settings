// Kundenreport als eigenstaendige HTML-Datei — zum Weitergeben oder als PDF
// drucken. Bewusst ohne externe Ressourcen: die Datei soll auch in fuenf Jahren
// im Anhang einer Mail noch so aussehen wie heute.
...
export function buildReportHtml(report) {
  ...
  const s = report.summary || { crit: 0, warn: 0, ok: 0, failedSections: [] }

  const sections = Object.entries(report.sections || {}).map(([id, sec]) => {
    if (!sec.ok) {
    ...
    return `<section><h2>${esc(sec.label)}</h2>
      <table>${(sec.metrics || []).map(metricRow).join('')}</table></section>`
  }).join('\n')
...
<title>M365-Statusbericht — ${esc(report.tenantName)}</title>
<style>
  ...
  * { box-sizing: border-box; }
  ...
  h2 { font-size: 1.05rem; border-bottom: 1px solid var(--rule); padding-bottom: .3rem; margin-bottom: .5rem; }
  table { width: 100%; border-collapse: collapse; font-size: .9rem; }
  td { padding: .35rem .5rem; border-bottom: 1px solid var(--rule); vertical-align: top; }
  td.ico { width: 1.5rem; font-weight: bold; }
  td.val { width: 6rem; text-align: right; font-variant-numeric: tabular-nums; }
  td.det { color: var(--dim); font-size: .82rem; }
...
<body>
<header>
  <h1>M365-Statusbericht</h1>
  <div class="meta"><strong>${esc(report.tenantName)}</strong>${report.organization ? ' · ' + esc(report.organization) : ''}<br />
