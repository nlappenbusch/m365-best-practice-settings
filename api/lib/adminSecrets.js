...
const ENV_SECRETS = [
  ["ADMIN_PASSWORD", "Anmeldung an diesem Werkzeug (Quelle der Wahrheit, überschreibt den gespeicherten Hash)", "hoch"],
  ["ANTHROPIC_API_KEY", "KI-Funktionen (Vorschläge, Maester-Erklärungen)", "mittel"],
  ["SDP_API_KEY", "ServiceDesk-Plus-Anbindung", "mittel"]
];
...
function collectSecrets(loadState, certPemPath) {
  ...
  if (s.sso && s.sso.clientSecret) {
    out.push({
      id: "sso:clientSecret", kind: "SSO-Clientgeheimnis", label: "Anmeldung über igeeks-Tenant",
  ...
  if (s.sessionSecret) {
    out.push({
      id: "state:sessionSecret", kind: "Sitzungsgeheimnis", label: "Signiert die Anmelde-Cookies",
  ...
  for (const [name, desc, severity] of ENV_SECRETS) {
    const v = process.env[name];
    out.push({
      id: "env:" + name, kind: "Umgebungsvariable", label: name, scope: "Prozess",
      recoverable: !!v, severity,
      meta: { "Zweck": desc, "Gesetzt": v ? "ja" : "nein", "Wert": v ? maskValue(v) : "—" },
      note: v ? null : "Nicht gesetzt."
    });
  }
  ...
  out.push({
    id: "info:mcpKeys", kind: "MCP-API-Keys", label: keys.length + " Key(s)",
  ...
  out.push({
    id: "info:adminPassword", kind: "Admin-Passwort", label: (s.auth && s.auth.username) || "admin",
